...
### 15.MyBatis结果集映射，如果是单个关联对象用什么映射,如果是一个集合的查询用什么映射
...
### 81.说说preparedStatement和Statement的区别
...
### 88.在MyBatis如何动态修改SQL（在代码执行的过程中修改SQL）
...
### 206.Spring,SpringBoot和SpringCloud的关系以及区别
...
### 234.你在项目中如何使用ZK实现分布式锁的？

工具curator

### 235.如果没有Redis和zookeeper怎么实现分布式锁？
...
### 236.你们数据库和Redis的数据一致性怎么做的

- 先删除缓存，再更新数据库，更新完后，再将新的数据刷回缓存redis

### 237.了解缓存击穿，穿透，雪崩吗？怎么处理？
...
### 242.说一下支付超时处理方案？延迟队列和死信队列是什么意思？

### 243.整个秒杀流程你用了几个队列，分别用来存储什么东西

- 延迟队列，存储秒杀到商品支付超时的订单
- 死信队列

### 244.秒杀成功，返回给用户的数据是什么？

### 245.你们怎么处理超卖，少卖的？

- 使用的是Hystrix的资源隔离的信号量手段来解决。信号量特性永远不会为负数

### 246.你知道BIO,NIO,AIO么？讲一下你的理解
...
### 248.创建线程的3种方式，  三种线程的区别？， start() 和 run()方法区别
...
### 250.线程的生命周期,  sleep 和 wait区别

- 生命周期：1.**new**创建一个线程    2.调用start方法启动线程进入一个**可运行的状态**	3.当线程争夺到cpu的执行权的时候就会进入到**运行状态**   4. 任务处理完后**销毁线程**      
...
### 251.synchronized 锁的原理，JDK1.6之后做了什么样的优化?
...
### 254.AtomicInterger怎么保证并发安全性的

- Atomic开头的类是基于CAS的乐观锁
- 使用version版本控制

### 255.什么是乐观锁，什么是悲观锁，什么是重入锁，什么是自旋锁，怎么是阻塞。
...
### 257.ThreadLocal的作用和原理？ThreadLocal的使用场景
...
### 261.EurekaClient拉取注册表&心跳续约用到了什么技术来实现？	

- 定时任务，使用到的是ScheduledThreadPool周期执行任务的线程池。默认是30s为周期

### 262.synchronized 和 volatile 的区别是什么？
...
### 264.数组和链表在内存中的存储结构有什么区别？

- 数组在内存中要求连续，链表不要求
- 数组做修改查询时性能比链表更好
- 链表做增加删除性能更好，因为数组内存要求连续，每次增加删除，数组需要移位操作

### 265.说一下散列存储(Hash存储) ， 什么是Hash冲突 ， 有什么解决方案
...
### 266.举例说说时间复杂度，比如：数组，链表，循环，嵌套循环 ， 2个非嵌套循环 ， 在什么情况下时间复杂度是什么？
...
### 268.你说一下树形结构和线性结构的优势？

- 树形结构树是连通的，只需要一次搜索就能完成遍历

### 269.说一下树的分类，以及你对它们的理解（二叉查找树的优缺点，平衡树的优缺点，红黑树的优缺点，B-树的优缺点 ，B+树的优缺点）平衡树 ，红黑树
...
### 280.HashMap在什么情况下扩容？HashMap如何扩容的？

- 扩载因子为0.75，当数组的长度达到扩载因子的时候就会进行2备扩容，将数组的长度x2
- 底层扩容原理就是建立一个新的数组，将老的数组的元素拷贝到新的数组中，然后再将每个元素重新计算hash值，重新获取该元素的索引位置

### 281.HashMap是如何Put一个元素的？
...
### 286.你知道HahsMap死循环问题吗？

- 在jdk1.8已经解决了死循环问题，在jdk1.7及以前存在死循环问题。主要原因是因为在之前HashMap链表插入的元素使用的是头插法，当HashMap扩容后，进行元素的重新计算位置排的时候就会出现链表上元素循环指向的问题
...
- HashMap的扩容阈值是0.75。也就是说初始化数组的长度是16，在元素达到12的时候，就会触发HashMap的扩容机制，就会新建一个HashMap，长度为之前的2倍，即32。然后将之前数组中的元素遍历取出来重新进行hash计算，重新计算元素的索引位置，然后进行存放在新建的HashMap中

### 291.常见的设计模式说一下，以及你用过的框架中哪儿用到这些设计模式。
...
### 305.mysql存储引擎有哪些，有什么区别，如何选择？

- InnoDB  支持事务，查询性能要低一点，不支持全文检索，一般使用ES作为全文检索
- MyIsam  不支持事务  查询性能高  支持全文检索
- Memory

### 306.一个sql ： select sum(amount) from recharge  ,来查询总充值，recharge  表数据量达到了上千万，怎么优化？
...
### 313.InnoDB辅助索引的叶子节点也存数据吗？

- 存放的是主键索引的键值，不存放数据

### 314.组合索引的匹配原则 ， 你优选使用单列索引和是组合索引，为什么？
...
### 328.水平分库分表后会出现哪些问题？怎么解决

- 分布式事务问题
- ID重复问题     使用雪花算法，让其ID不重复，也可以单独建立一张自增长的ID表来维护分表的ID

### 329.你们公司使用的是什么技术来水平分表？还可以有什么技术？有什么区别？

### 330.你们使用什么规则来分库分表的？还有哪些规则？

### 331.你说一下数据量从 100 到 1000万 到 上亿，你按照什么样的顺序去优化你的数据库？
...
### 338.能简单说一下你怎么使用shardingjdbc做读写分离的嘛

- 项目中导入依赖
- 配置一个propertites配置文件，里面配置master和slave主数据库和从数据库的名称以及连接的信息username和密码

### 339.能简单说一下你怎么使用shardingjdbc做读分库分表的嘛

- 在多个服务器安装数据库，然后设置配置，安装路径，端口等，在通过一些命令完成分库
- 分表通过雪花算法计算ID水平分表

### 340.Redis的主从有什么优点，和缺点？

### 341.解释一下Redis的哨兵模式。哨兵的不足？

### 342.Redis的cluster集群怎么存储数据的？
...
### 345.Redis存储结构底层有没有了解？什么是SDS

### 346.Redis如何模拟队列和栈，用什么命令

- list存储结构的lpush,rpush 以及lpop和rpopmini队列和栈

### 347.Redis存储单个对象怎么存，存储对象集合怎么存

- 单个对象  String
- 对象集合  hash

### 348.你们Redis用来做什么？使用的什么结构？
...
### 359.简单说一下主备切换时Redis如何选举master的？ 
...
### 363.Redis主从有什么优缺点，哨兵有什么优缺点，cluster集群有什么优缺点？
...
### 364.Redis集群有多少个槽位？它是怎么计算一个key该存储到哪个Redis节点上的？
...
### 372.Mysql中的数据库，表，行，列 在ES中是怎么对应的
...
### 373.如果我要把一个User表中的数据存储到ES中，我应该怎么做
...
### 375.详细描述一下Elasticsearch获取文档的过程
...
### 377.详细描述一下Elasticsearch更新和删除文档的过程

### 378.ES有几种节点类型？他们的作用分别是什么
